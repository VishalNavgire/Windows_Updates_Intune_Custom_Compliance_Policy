...
.Requirement
    1. Microsoft Entra App needs to be registered in to your Tenant first with permission type as 'Delegated'. After you complete App's registration please 
        update App's ID. Search variable name 👉  $Registered_Entra_App_ID to enter MS Entra Registered App ID here. To read more on how to create / register an MS Entra App Id with Delegated rights - https://learn.microsoft.com/en-us/graph/auth-register-app-v2#register-an-application 
...
$Registered_Entra_App_ID = Read-Host "Please enter Microsoft Entra Registered App ID here without single or double quotes `n"
        ...
        Try 
            {
                $Win32AppDeployment.value | ForEach-Object {
                    ...
                    $App_Assign_Source = If ($Name.Assignments) 
                                                        {
                                                            ($Name.Assignments | ForEach-Object {$_.Source}) -Join "; "
                                                        } 
                                                    Else 
                                                        {
                                                            "No Results"
                    ...
                    $App_Assign_Source_ID = If ($Name.Assignments) 
                                                        {
                                                            ($Name.Assignments | ForEach-Object {$_.SourceID}) -Join "; "
                                                        } 
                                                    Else 
                                                        {
                                                            "No Results"
                    ...
                    $DeviceAndAppManagementAssignmentFilterId =  If ($Name.Assignments) 
                            {   
                                (
                                    $Name.Assignments | ForEach-Object { 
                                                                            If ($_.Target.deviceAndAppManagementAssignmentFilterId) 
                                                                                {
                                                                                    Return $($_.Target.deviceAndAppManagementAssignmentFilterId)
                                                                                } 
                                                                            Else { "Not Available"}              
                 ...
                 $DeviceAndAppManagementAssignmentFilterType =  If ($Name.Assignments) 
                            {   
                                (
                                    $Name.Assignments | ForEach-Object { 
                                                                            If ($_.Target.deviceAndAppManagementAssignmentFilterType) 
                                                                                {
                                                                                    Return $($_.Target.deviceAndAppManagementAssignmentFilterType)
                                                                                } 
                                                                            Else { "Not Available"}              
                ...
                $App_Assignment_Settings = If ($Name.Assignments) 
                                                    {
                                                        # ($Name.Assignments | ForEach-Object {("$($_.vpnConfigurationId), $($_.uninstallOnDeviceRemoval), $($_.isRemovable), $($_.preventManagedAppBackup)")}) -Join "; "
                                                        ($Name.Assignments| ForEach-Object {
                                                                                            If ($_.Settings)
                                                                                            # {("$($_.vpnConfigurationId), $($_.uninstallOnDeviceRemoval), $($_.isRemovable), $($_.preventManagedAppBackup)")}
                                                                                            {$_.Settings}
                                                                                            Else { "Not Available"}
                    ...
                    $Installation_Exprience = If ($Name.InstallExperience) 
                                                {
                                                        (
                                                            $Name.InstallExperience | ForEach-Object{
                                                                                                        ("$($_.runAsAccount), $($_.maxRunTimeInMinutes), $($_.deviceRestartBehavior)")
                                                                                                    }
                                                        ) -Join "; "
                                                }
                                                Else { "Not Available"}
                    $Detection_Criteria = If ($Name.DetectionRules)
                                            {
                                                ($Name.DetectionRules | ForEach-Object {
                                                                                        "$($_.'@odata.type'), $($_.productCode), $($_.productVersionOperator), $($_.productVersion)"
                                                                                        }) -join "; "
                                            }
                                            Else { "Not Available"}

                     $Return_Codes = If ($Name.ReturnCodes)
                                            {
                                                ($Name.ReturnCodes | ForEach-Object { "$($_.ReturnCode)=$($_.Type)"}) -join "; "
                                            }

                    $Allow_Available_App_Uninstall = ($Name.AllowAvailableUninstall)
                    $AllWin32AppDeployment += [PSCustomObject] @{
                                                                    App_Name          = ($Name.DisplayName).ToUpper()
                                                                    App_Description = ($Name.Description).ToUpper()
                                                                    App_Publisher = ($Name.Publisher).ToUpper()
                                                                    App_Assign_Source = $App_Assign_Source
                                                                    App_Assign_Source_ID = $App_Assign_Source_ID
                                                                    App_IsFeatured = ($Name.isFeatured)
                                                                    App_privacyInformationUrl = ($Name.privacyInformationUrl)
                                                                    App_informationUrl = ($Name.informationUrl)
                                                                    App_Owner = ($Name.Owner)
                                                                    App_developer = ($Name.developer)
                                                                    App_Note = ($Name.Notes)
                                                                    App_ApplicableDeviceType = ($Name.ApplicableDeviceType)
                                                                    App_StoreUrl = ($Name.AppStoreURL)
                                                                    ...
                                                                    # App_Size_GB        = If ($Name.Size) {[math]::Round($Name.Size / 1GB,2)}
                                                                    ...
                                                                    Allow_App_Available_Uninstall = $Allow_Available_App_Uninstall
                                                                    App_Installation_Exprience = $Installation_Exprience
                                                                    App_Detection_Criteria = $Detection_Criteria
                                                                    App_ReturnCodes = $Return_Codes
                                                                    ...
                                                                    App_DeviceAndAppManagementAssignmentFilterId = $DeviceAndAppManagementAssignmentFilterId
                                                                    App_DeviceAndAppManagementAssignmentFilterType = $DeviceAndAppManagementAssignmentFilterType
                                                                    App_Assignment_Settings = $App_Assignment_Settings
